...
DOPAMINE = _ROOT / "examples" / "spectra" / "Dopamine_Powder.CSV"
V7 = _ROOT / "models" / "struct_fg_v7_pubchem_mordred.joblib"


@pytest.fixture
def dopamine_spectrum():
    from lib.spectrum import load_processed_spectrum

    if not DOPAMINE.is_file():
        pytest.skip("Dopamine_Powder.CSV missing")
    return load_processed_spectrum(DOPAMINE)


def test_evidence_json_serializable(dopamine_spectrum):
    from ml.ftir_evidence import extract_spectral_evidence

    ev = extract_spectral_evidence(dopamine_spectrum.wn, dopamine_spectrum.y)
    json.dumps(ev)
    assert "band_matches" in ev and "peaks" in ev


def test_rule_assignment_structure(dopamine_spectrum):
    from ml.ftir_evidence import extract_spectral_evidence
    from ml.ftir_rules import assign_functional_groups_from_evidence

    rules = assign_functional_groups_from_evidence(
        extract_spectral_evidence(dopamine_spectrum.wn, dopamine_spectrum.y)
    )
...
def test_ml_refinement_none(dopamine_spectrum):
    from ml.ftir_evidence import extract_spectral_evidence
...
def test_ml_missing_model_path_graceful(dopamine_spectrum):
...
def test_fusion_modes(dopamine_spectrum, fusion_mode):
...
def test_dopamine_no_silicone_nitro_top_hits(dopamine_spectrum):
    from ml.ftir_pipeline import run_evidence_first_pipeline

    pipe = run_evidence_first_pipeline(dopamine_spectrum.wn, dopamine_spectrum.y, ml_mode="none")
...
def test_language_avoids_certainty_claims(dopamine_spectrum):
    from ml.ftir_pipeline import run_evidence_first_pipeline
